...
func TestWireInjectionDeterminism(t *testing.T) {
	...
	for _, kind := range []string{"openai", "anthropic"} {
		build := func(r RouterReplayRequest, inj *uuidInjection) []byte {
			...
			var err error
			if kind == "openai" {
				body, _, err = buildOpenAIChatCompletionsBody(r, docs, "model", "", 0, 0, false, 0, inj, "", "")
			} else {
				body, _, err = buildAnthropicMessagesBody(r, docs, "model", "", 0, 0, false, 0, inj)
			}
			if err != nil {
...
func TestInlineMarkerAppendedToTurnMessage(t *testing.T) {
	...
	body, canonical, err := buildAnthropicMessagesBody(req, docs, "model", "", 0, 0, false, 0, inj)
	if err != nil {
...
func TestCacheFidelitySharedBlockUnaffectedByInjection(t *testing.T) {
	...
	bodyNoInj, _, err := buildAnthropicMessagesBody(req, docs, "model", "", 0, 0, false, 0, nil)
	if err != nil {
	...
	bodyWithInj, _, err := buildAnthropicMessagesBody(req, docs, "model", "", 0, 0, false, 0, inj)
	if err != nil {
