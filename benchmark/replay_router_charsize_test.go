...
func TestBuildAnthropicMessagesBodyCharsPerToken(t *testing.T) {
	...
	byteBody, _, err := buildAnthropicMessagesBody(req, docs, "model", "", 0, 0, false, 0, nil)
	if err != nil {
	...
	tokBody, _, err := buildAnthropicMessagesBody(req, docs, "model", "", 0, 0, false, charsPerToken, nil)
	if err != nil {
	...
	tokBody2, _, err := buildAnthropicMessagesBody(req, docs, "model", "", 0, 0, false, charsPerToken, nil)
	if err != nil {
...
func TestBuildOpenAIChatCompletionsBodyCharsPerToken(t *testing.T) {
	...
	byteBody, _, err := buildOpenAIChatCompletionsBody(req, docs, "model", "", 0, 0, false, 0, nil, "", "")
	if err != nil {
		t.Fatalf("build (charsPerToken=0): %v", err)
	}
	...
	tokBody, _, err := buildOpenAIChatCompletionsBody(req, docs, "model", "", 0, 0, false, charsPerToken, nil, "", "")
	if err != nil {
		t.Fatalf("build (charsPerToken=3.4): %v", err)
	}
