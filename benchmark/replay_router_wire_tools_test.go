...
func TestOpenAIVsAnthropicBodySize(t *testing.T) {
	...
	anthBody, _, err := buildAnthropicMessagesBody(req, docs, "model-a", "", 0, false, nil)
	if err != nil {
		t.Fatalf("buildAnthropicMessagesBody: %v", err)
	}
	openaiBody, _, err := buildOpenAIChatCompletionsBody(req, docs, "model-a", "", 0, false, nil)
	if err != nil {
	...
	for sessCount < maxSessions && scanner.Scan() {
		...
		for _, inst := range sess.Instances {
			for _, r := range inst.Requests {
				ab, _, err := buildAnthropicMessagesBody(r, docs, "model-a", "", 0, false, nil)
				if err != nil {
					continue
				}
				ob, _, err := buildOpenAIChatCompletionsBody(r, docs, "model-a", "", 0, false, nil)
				if err != nil {
					continue
				}
...
func TestOpenAIToolUseConversion(t *testing.T) {
	...
	body, _, err := buildOpenAIChatCompletionsBody(req, docs, "model-x", "", 0, false, nil)
	if err != nil {
		t.Fatalf("buildOpenAIChatCompletionsBody: %v", err)
	}
	...
	orphanBody, _, err := buildOpenAIChatCompletionsBody(reqOrphan, docs, "model-x", "", 0, false, nil)
	if err != nil {
