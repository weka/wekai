...
type BenchmarkAutoOptions struct {
	...
	ReplayCharsPerToken        float64  `long:"replay-chars-per-token" description:"Router-replay only: sizes synthesized replay content as block Tokens x this many chars instead of the capture's byte sizes, so the serving tokenizer's counts land near the ORIGINAL capture's token counts. 0 = off (default: byte-faithful sizing from the capture's recorded Bytes). Superseded per-block by --replay-exact-tokens when that's also set." default:"0" env:"BENCHMARK_REPLAY_CHARS_PER_TOKEN"`
	ReplayExactTokens          bool     `long:"replay-exact-tokens" description:"Router-replay only: sizes synthesized replay content by binary-searching a corpus token index built once at startup against the FIRST target endpoint's own POST /tokenize, instead of a fixed chars/token ratio -- lands exactly (not approximately) on each block's captured token count. Requires a working /tokenize on that endpoint; fails the run fast at startup otherwise (fall back to --replay-chars-per-token, or drop both for byte-faithful sizing)." env:"BENCHMARK_REPLAY_EXACT_TOKENS"`
