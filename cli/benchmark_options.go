...
type BenchmarkAutoOptions struct {
	...
	EndpointOverloadThreshold  float64  `long:"endpoint-overload-threshold" default:"1.5" env:"BENCHMARK_ENDPOINT_OVERLOAD_THRESHOLD" description:"Multi-endpoint specs only (a|b|c): a series sticks to its assigned endpoint until that endpoint's in-flight requests exceed this multiple of its fair share (total concurrency / endpoints), then the request fails over to a deterministically hash-selected endpoint that is not overloaded. 1.5 = default."`
