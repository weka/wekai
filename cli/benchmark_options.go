...
type BenchmarkAutoOptions struct {
	...
	VLLMMetricsURLs               []string      `long:"vllm-metrics-url" description:"Where to scrape vllm:prompt_tokens_by_source_total for the cache-source breakdown; repeatable. Needed whenever the thing serving inference is not the thing exposing the counters — behind a router it never is, because the router refuses /metrics on its serving port on purpose (proxying it answers with ONE backend's counters, which looks like a fleet total and is worse than nothing) and serves the aggregate on its metrics listener instead. Point this at that listener (router run with --vllm-metrics) or at every backend directly; totals are summed as per-endpoint deltas either way, so restarts and departures do not corrupt them. Accepts a bare endpoint or a full /metrics URL. Default: derived from --models, which is right only when serving it directly." env:"BENCHMARK_VLLM_METRICS_URL"`
	TTFTLimitStat                 string        `long:"ttft-limit-stat" choice:"mean" choice:"p50" choice:"p95" choice:"p99" default:"mean" description:"Which statistic over the --ttft-window the admission gate compares against --ttft-limit. This DECIDES the answer rather than decorating it: TTFT on this workload is heavily skewed, so a mean gate is tail-driven and closes at roughly half the session count a p50 gate would. All four are reported in the progress line whichever one gates, so a single run says what its plateau would have been under each -- the alternative is one eight-hour arm per statistic."`
	ReplayReuseSessions           string        `long:"replay-reuse-sessions" choice:"true" choice:"false" default:"true" optional:"yes" optional-value:"true" description:"Replay the corpus again from the top instead of draining, so a run can outlast its own dataset -- 5,441 sessions at this workload's turnover is about 3.3 hours, and an 8-hour arm is otherwise impossible. ON by default, because the two ways a run ends up short of its dataset both end in a number that reads as the fleet's: a long arm drains it, and a session count above the corpus size leaves the surplus slots empty from the first minute. Each pass gets its OWN content stamp, applied uniformly across the whole pass: within a pass two sessions that shared a system prompt in the capture still share it, so the sharing topology is identical to pass one; across passes the keyspace is disjoint, so pass two cannot hit pass one's cache entries and inflate the hit rate. A cache figure spanning several passes is not comparable to a single-pass one; the run summary reports the pass count. Pass --replay-reuse-sessions=false to sweep the corpus once and stop at the end of it -- note that a run doing that ends when the corpus drains, which is the only terminator a run with no --timeout and no --total has." env:"BENCHMARK_REPLAY_REUSE_SESSIONS"`
	ReplayAllowUnderfill          bool          `long:"replay-allow-underfill" description:"Let a --replay-realtime run continue after the corpus runs out and admitted session slots can no longer be filled. Off by default, so the run ABORTS. Aborting is the point: idle slots keep counting toward the governor session number while the fleet does less work, so the reported concurrency stays authoritative as the thing beneath it decays. Nothing else in the output moves either -- a session that never starts is not late, so neither the lateness nor the fidelity fields catch it."`
	ReplayRealtime                bool          `long:"replay-realtime" description:"Router-replay only: pace each session by the timestamps it was captured with, so a conversation keeps its own think time instead of firing back-to-back. Requests are floored by the previous response — a session cannot ask turn i+1 before it has answer i — so on a fleet slower than the capture the session falls behind rather than issuing a backlog." env:"BENCHMARK_REPLAY_REALTIME"`
	AdmitEvery                    time.Duration `long:"admit-every" description:"Start one session, then admit one more this often while the fleet keeps up. Load grows by adding SESSIONS, so concurrency is whatever the fleet's latency implies rather than a number you set. NOT a neutral knob: on a fleet that answers fast this rate, not the fleet, becomes the limit — at a 0.90 cache hit rate one-per-second finds 52.8 req/s at 36% prefill utilisation while five-per-second finds 140.7 at 97%. Run two rates and check the answer stopped moving. 0 = off, use the fixed series pool." env:"BENCHMARK_ADMIT_EVERY"`
	TTFTLimit                     time.Duration `long:"ttft-limit" default:"5s" description:"Admission pauses while the windowed mean TTFT is at or above this, and resumes below it. This is the definition of 'sustained' for the run: max sessions at 5s TTFT and at 1s TTFT are different capacity numbers for the same fleet, so it belongs in the run record whether or not you change it." env:"BENCHMARK_TTFT_LIMIT"`
	TTFTWindowDur                 time.Duration `long:"ttft-window" default:"30s" description:"How far back the admission gate averages TTFT, and it is the ARITHMETIC MEAN over that window — not the median, not a percentile. That choice is deliberate and it decides the answer: on this workload TTFT is heavily skewed (median 4.3s against a mean 8.7s, p99 52s), so a mean gate is tail-driven rather than typical-case-driven and closes at roughly half the session count a median gate would. One 193s request in a 30s window of 300 moves the mean 0.64s by itself. The tail is real callers waiting, and a fleet whose p99 is 52s is not comfortably carrying that load however healthy its median looks. A duration rather than a request count for a related reason: a count is a different amount of history at every load level, so a count-based gate tightens as the fleet fills — exactly where it has to stay comparable." env:"BENCHMARK_TTFT_WINDOW"`
	ReplaySkipIdle                bool          `long:"replay-skip-idle" description:"With --replay-realtime, jump the clock forward when NOTHING is in flight and every active session is waiting on its next turn. Keeps a replay from spending its wall-clock budget on think time (captured median gap ~8s, mean ~4min). Safe because the condition requires an idle fleet: with no request in flight, none can have its latency distorted. Rarely fires once the run is loaded, which is the intent." env:"BENCHMARK_REPLAY_SKIP_IDLE"`
	...
	Verify                        bool          `long:"verify" description:"Verify cache coherency during the run: inject one deterministic marker per user turn into --router-replay-file replay sessions, ask later responses to recite a bounded window of them, and scan every response for a marker it never sent. Reports PRESENCE_MISS (the model did not repeat its own marker) and CROSS_CONTAMINATION (a response carried another session's marker) alongside the throughput numbers. ROUTER-REPLAY ONLY: the run fails at startup under any other mode." env:"BENCHMARK_VERIFY"`
	VerifyContinueOnContamination bool          `long:"verify-continue-on-contamination" description:"Keep the benchmark running after --verify finds a leaked marker. By default the run STOPS on the first cross-contamination event, freezing the fleet in the state that produced it; either way the event prints in full (both sessions, the marker, and the exact --replay-series-indices pair that reproduces it)." env:"BENCHMARK_VERIFY_CONTINUE_ON_CONTAMINATION"`
	DumpDir                       string        `long:"dump-dir" description:"Write every exchange verbatim to this directory: <name>.request.json is the exact bytes posted, <name>.response.raw the exact bytes received (SSE framing included), and <name>.meta.json the scoring derived from them. Intended for diagnosing a validation result -- a PRESENCE_MISS beside a request that never carried the marker is a different defect from one beside a response that ignored it. Replay prompts run to hundreds of KB each, so pair it with --total." env:"BENCHMARK_DUMP_DIR"`
	DumpLimit                     int           `long:"dump-limit" description:"Maximum exchanges --dump-dir writes before it stops, announced on stderr when reached (default 200). A directory that stops growing otherwise looks like a run that stopped making requests." default:"200" env:"BENCHMARK_DUMP_LIMIT"`
	Seed                          int64         `long:"seed" description:"Seed for this run's identity. The run-id stamped into every prompt is derived from it, and so is everything derived from that -- including --verify's coherency markers. 0 (default) draws a fresh seed and prints it. WARNING: reusing a seed reproduces this run's content exactly, so an earlier run with the same seed left these prefixes on the fleet and the cache hit rate measures its leftovers rather than a cold start." default:"0" env:"BENCHMARK_SEED"`
	...
	VerifyForceEOS                bool          `long:"verify-force-eos" description:"With --verify, keep vLLM's ignore_eos ON as in a regular benchmark run. By default --verify turns it off: forcing generation past the stop token pads every response with degenerate text, which is exactly the noise a coherency check does not want to score. Off-eos verify holds ~90% output conformity via the built-in per-request length ask; pass this flag when deterministic output volume matters more than genuine output." env:"BENCHMARK_VERIFY_FORCE_EOS"`
...
// flagOn reads a default-true boolean flag — a string carrying choice:"true"
// choice:"false" default:"true" optional-value:"true".
//
// A string rather than a bool because go-flags refuses a default on a boolean
// switch, and all three states have to be reachable: unset means ON, an explicit
// false has to reach the run, and the bare flag has to keep meaning what it
// meant when it was a switch. Only "false" is off, so a zero value that never
// went through the parser reads as the default rather than as its opposite.
func flagOn(v string) bool { return v != "false" }

// ReuseSessions is --replay-reuse-sessions with its default applied: cycle the
// corpus unless the run explicitly asked for a single sweep.
func (c *BenchmarkAutoOptions) ReuseSessions() bool { return flagOn(c.ReplayReuseSessions) }

// FIFOGateOrder is --random-gate-order inverted: the legacy exact-FIFO wake
// order is what you get by turning the default-on random order off.
func (c *BenchmarkAutoOptions) FIFOGateOrder() bool { return !flagOn(c.RandomGateOrder) }
