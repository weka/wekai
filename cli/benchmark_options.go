...
type BenchmarkAutoOptions struct {
	...
	ReplayReuseSessions        bool          `long:"replay-reuse-sessions" description:"Replay the corpus again from the top instead of draining, so a run can outlast its own dataset -- 5,441 sessions at this workload's turnover is about 3.3 hours, and an 8-hour arm is otherwise impossible. Each pass gets its OWN content stamp, applied uniformly across the whole pass: within a pass two sessions that shared a system prompt in the capture still share it, so the sharing topology is identical to pass one; across passes the keyspace is disjoint, so pass two cannot hit pass one's cache entries and inflate the hit rate. A cache figure spanning several passes is not comparable to a single-pass one; the run summary reports the pass count."`
	...
	ReplayInjectUUIDs          bool          `long:"replay-inject-uuids" description:"Inject one deterministic UUID per user turn into --router-replay-file replay sessions (spread through the conversation), and validate a windowed recite of first + up to 3 most recent turns (excluding the current turn) in later responses -- a coherency check (PRESENCE_MISS / CROSS_CONTAMINATION) for the KV-offload path under realistic multi-turn agentic traffic. ROUTER-REPLAY PATH ONLY: requires --router-replay-file; rejected together with --from-dataset." env:"BENCHMARK_REPLAY_INJECT_UUIDS"`
	ReplayUUIDSeed             int64         `long:"replay-uuid-seed" description:"PRNG seed for --replay-inject-uuids' per-session UUID generation (0 = crypto/rand, non-deterministic across runs)." default:"0" env:"BENCHMARK_REPLAY_UUID_SEED"`
	// ReplayReciteEveryRequest is a string (not bool) choice, same workaround
	// as RandomGateOrder above: a plain bool flag defaulting true can never
	// be turned OFF via the CLI with this parser (go-flags only accepts
	// `--flag=false` for bool options when the parser is built with
	// AllowBoolValues, which main.go's flags.Default does not set).
	ReplayReciteEveryRequest  string  `long:"replay-recite-every-request" choice:"true" choice:"false" default:"true" optional:"yes" optional-value:"true" description:"With --replay-inject-uuids, ask the model to recite the ref-id marker on EVERY request (the DEFAULT) rather than only on each agent instance's final request. Pass --replay-recite-every-request=false to recite only on each instance's last request." env:"BENCHMARK_REPLAY_RECITE_EVERY_REQUEST"`
