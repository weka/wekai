...
type BenchmarkAutoOptions struct {
	...
	LimitContext               int      `long:"limit-context" description:"Replay only: skip any request whose built body exceeds this many TOKENS (approximated as tokens*4 characters). Use to avoid 400 storms when replaying long-session captures against a small-context model. 0 = no limit." env:"BENCHMARK_LIMIT_CONTEXT"`
