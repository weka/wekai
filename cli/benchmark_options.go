...
type BenchmarkAutoOptions struct {
	...
	TTFTWindowDur              time.Duration `long:"ttft-window" default:"30s" description:"How far back the admission gate averages TTFT. A duration rather than a request count on purpose: a count is a different amount of history at every load level, so a count-based gate tightens as the fleet fills — exactly where it has to stay comparable." env:"BENCHMARK_TTFT_WINDOW"`
