...
// BenchmarkEmbedCommand implements the benchmark embed subcommand
type BenchmarkEmbedCommand struct {
	*BenchmarkEmbedOptions
}

func (c *BenchmarkEmbedCommand) Execute(args []string) error {
	...
	if config.Config.GlobalModelOverride != "" {
		return fmt.Errorf("--global-model-override cannot be used with benchmark command; specify models via --models flag")
	}
	...
	// Initialize context with unified initialization
	ctx := context.Background()
	ctx, shutdown, err := config.Init(ctx, "benchmark")
	if err != nil {
	...
	go func() {
		...
		if !config.Config.JSONOutput {
			fmt.Printf("\n\nReceived signal %v. Stopping benchmark and printing partial results...\n\n", sig)
		}
		cancel() // Cancel the context to stop all ongoing work
	}()
	...
	if c.Timeout != "" {
		...
		if !config.Config.JSONOutput {
			fmt.Printf("  Timeout: %v (will print partial results if exceeded)\n", duration)
		}
	}
	...
	// Print configuration (unless in JSON output mode)
	if !config.Config.JSONOutput {
		totalRequests := c.Cycles * c.Series * c.NumRequests
		fmt.Printf("Benchmark Configuration:\n")
		fmt.Printf("  Documentation: %s\n", docsDir)
		fmt.Printf("  Models: %v\n", c.Models)
		fmt.Printf("  Cycles: %d\n", c.Cycles)
		fmt.Printf("  Series per cycle: %d\n", c.Series)
		fmt.Printf("  Concurrency: %d (series running in parallel within each cycle)\n", c.Concurrency)
		fmt.Printf("  Requests per series: %d\n", c.NumRequests)
		fmt.Printf("  Total requests per model: %d (cycles × series × requests)\n", totalRequests)
		fmt.Printf("  Sleep between requests: %d seconds\n", c.SleepBetween)
	...
	if c.Concurrency > c.Series {
		return fmt.Errorf("concurrency (%d) cannot exceed number of series (%d)", c.Concurrency, c.Series)
	}

	// Create benchmark request
	baseReq := benchmark.EmbedBenchmarkRequest{
		DocsDir:      docsDir,
		DocsContent:  docsContent,
		Question:     question,
		NumRequests:  c.NumRequests,
		SleepBetween: time.Duration(c.SleepBetween) * time.Second,
		NumSeries:    c.Series,
		NumCycles:    c.Cycles,
		Concurrency:  c.Concurrency,
	}

	// Progress callback: stdout in text mode, stderr in JSON mode
	...
	// Run benchmark in parallel across all models
	result, err := benchmark.RunBenchmarkParallel(ctx, c.Models, baseReq, progressCallback)
	if err != nil {
		if config.Config.JSONOutput {
...
// parseTokenSize parses a token count string, supporting k/m suffixes (e.g., "3k" → 3000).
